...
static void save(char* filename, nat sizeof_filename) {
	...
	close(file);
	printf("\033[7minfo: saved %llu bytes\033[0m\n", count);
}
...
loop:;	char c = 0;
	...
	if (0) {}
		...
		for (nat i = 0; i < count; i++) {
			if (t == tofind_count) {
				printf("\033[7mat:%llu..%llu\033[0m\n", i - tofind_count, i);
				selection_count = tofind_count;
				selection_begin = i; cursor = i; anchor = i;
