...
struct action {
    ...
    struct location pre_visual_cursor;
    struct location pre_visual_origin;
    struct screen_location pre_visual_screen;
    i32 pre_visual_desired;
    
    struct location post_begin;
    struct location post_cursor;
    struct location post_render_cursor;
    struct location post_visual_cursor;
    struct location post_visual_origin;
    struct screen_location post_visual_screen;
    i32 post_visual_desired;
...
static inline void insert(u8 c, i8 record_action) {
    ...
    if (record_action) {
        ...
        new_action->pre_begin = file->begin;
        new_action->pre_cursor = file->cursor;
        new_action->pre_render_cursor = file->render_cursor;
        new_action->pre_visual_cursor = file->visual_cursor;
        new_action->pre_visual_screen = file->visual_screen;
        new_action->pre_visual_origin = file->visual_origin;
        new_action->pre_visual_desired = file->visual_desired;
        new_action->pre_saved = file->saved;
    }
...
final:
    ...
    if (record_action and ((c >> 6) != 2)) {
                        
        new_action->length = 1;
        new_action->text = malloc(1);
        new_action->text[0] = c;
        
        new_action->post_begin = file->begin;
        new_action->post_cursor = file->cursor;
        new_action->post_render_cursor = file->render_cursor;
        new_action->post_visual_cursor = file->visual_cursor;
        new_action->post_visual_screen = file->visual_screen;
        new_action->post_visual_origin = file->visual_origin;
        new_action->post_visual_desired = file->visual_desired;
        new_action->post_saved = file->saved;
    ...
    } else if (record_action and not ((c >> 6) != 2)) {
        
        file->head->post_begin = file->begin;
        file->head->post_cursor = file->cursor;
        file->head->post_render_cursor = file->render_cursor;
        file->head->post_visual_cursor = file->visual_cursor;
        file->head->post_visual_screen = file->visual_screen;
        file->head->post_visual_origin = file->visual_origin;
        file->head->post_visual_desired = file->visual_desired;
        
        file->head->text = realloc(file->head->text, (size_t)(file->head->length + 1));
        file->head->text[file->head->length++] = c;
    }
...
static inline void backspace(i8 record_action) {
    ...
    if (record_action) {
        new_action = calloc(1, sizeof(struct action));
        new_action->type = backspace_action;
        new_action->parent = file->head;
    ...
    if (record_action) {
        new_action->length = copy_deleted_length;
        new_action->text = copy_deleted;
...
static inline void destroy_tree(struct action* node) {
    for (i16 i = 0; i < node->count; i++)
...
static inline void delete_selection() {
    ...
    new->type = delete_text_action;
    new->parent = file->head;
        
    new->pre_begin = file->begin;
    new->pre_cursor = file->cursor;
    new->pre_render_cursor = file->render_cursor;
    new->pre_visual_cursor = file->visual_cursor;
    new->pre_visual_screen = file->visual_screen;
    new->pre_visual_origin = file->visual_origin;
    new->pre_visual_desired = file->visual_desired;
    new->pre_saved = file->saved;
    ...
    new->post_cursor = file->cursor;
    new->post_render_cursor = file->render_cursor;
    new->post_visual_cursor = file->visual_cursor;
    new->post_visual_screen = file->visual_screen;
    new->post_visual_origin = file->visual_origin;
    new->post_visual_desired = file->visual_desired;
    new->post_saved = file->saved;
...
static inline void set_begin() {
    ...
    struct action* new = calloc(1, sizeof(struct action));     ///TODO: finish implementing undo and redo.
...
static inline void insert_text(const char* string) { 
    ...
    for (i32 i = 0; i < new->length; i++) insert((u8) string[i], 0);
...
static inline const char* action_name(i8 action_type) {
    ...
    if (action_type == insert_action) return "insert";
    if (action_type == backspace_action) return "backspace";
    if (action_type == delete_text_action) return "delete text";
    if (action_type == paste_text_action) return "paste text";
    if (action_type == set_anchor_action) return "set anchor";
...
static inline void replay_action() {
    ...
    file->begin = file->head->pre_begin;
    file->cursor = file->head->pre_cursor;
    file->render_cursor = file->head->pre_render_cursor;
    file->visual_cursor = file->head->pre_visual_cursor;
    file->visual_screen = file->head->pre_visual_screen;
    file->visual_origin = file->head->pre_visual_origin;
    file->visual_desired = file->head->pre_visual_desired;
    file->saved = file->head->pre_saved;
        
    if (file->head->type == no_action) {
        // do nothing.
    } else if (file->head->type == backspace_action) {
        backspace(0);
        
    } else if (file->head->type == insert_action) {
        for (i32 i = 0; i < file->head->length; i++)
            insert(file->head->text[i], 0);
        
    } else if (file->head->type == set_anchor_action) {
        file->begin = file->head->post_begin;
        
    } else if (file->head->type == paste_text_action) {
        for (i32 i = 0; i < file->head->length; i++)
            insert(file->head->text[i], 0);
        
    } else if (file->head->type == delete_text_action) {
        if (behind(file->cursor, file->head->pre_begin)) {
            struct location save = file->cursor;
            while (file->cursor.line < file->head->pre_begin.line or
                   file->cursor.column < file->head->pre_begin.column)
                move_right(0);
            while (save.line < file->cursor.line or
                   save.column < file->cursor.column)
                backspace(0);
        } else {
            while (file->head->pre_begin.line < file->cursor.line or
                   file->head->pre_begin.column < file->cursor.column)
                backspace(0);
        }
    }
    
    file->begin = file->head->post_begin;
    file->cursor = file->head->post_cursor;
    file->render_cursor = file->head->post_render_cursor;
    file->visual_cursor = file->head->post_visual_cursor;
    file->visual_screen = file->head->post_visual_screen;
    file->visual_origin = file->head->post_visual_origin;
    file->visual_desired = file->head->post_visual_desired;
    file->saved = file->head->post_saved;
}
...
static inline void reverse_action() {
    ...
    if (file->head->type == no_action) {
        ...
        while (file->head->pre_cursor.line < file->cursor.line or
               file->head->pre_cursor.column < file->cursor.column)
            backspace(0);
