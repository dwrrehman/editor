...
static const char* autosave_directory = "/home/pi/documents/autosaves/";
...
static inline void display() {
    ...
    for (size_t line = file->visual_origin.line; line < limit; line++) {
        ...
        if (file->options.show_line_numbers) {
            if (not file->render.lines[line].continued) length += sprintf(window.screen + length, "\033[38;5;%lum" "%*lu" "\033[0m"  "  " , (unsigned long)59UL, (int)file->line_number_width, (unsigned long)++line_number);
        ...
        length += sprintf(window.screen + length, "\033[%lu;1H", (unsigned long)++screen_line);
    }
    ...
    if (file->options.show_status) {
        ...
        length += sprintf(window.screen + length, "\033[%lu;1H", (unsigned long)window.rows + 1);
        length += sprintf(window.screen + length, "\033[7m"  "\033[38;5;%lum" "%s" "\033[m", (unsigned long)252L, status);
    }
            
    length += sprintf(window.screen + length, "\033[%lu;%luH",
                      (unsigned long)file->visual_screen.line + 1,
                      (unsigned long)file->visual_screen.column + 1 + file->line_number_cursor_offset);
...
static inline void textbox_display(struct textbox* box, const char* prompt, long color) {
    size_t length = sprintf(window.screen, "\033[%lu;1H" "\033[38;5;%lum"  "%s" "\033[m", (unsigned long)window.rows, (unsigned long)color, prompt);
    ...
    length += sprintf(window.screen + length, "\033[%lu;%luH", (unsigned long)window.rows, (unsigned long)box->visual_screen + 1 + box->prompt_length);
    write(STDOUT_FILENO, window.screen, length);
}
...
static inline void print_above_textbox(char *message, long color) {
    size_t length = sprintf(window.screen, "\033[%lu;1H" "\033[38;5;%lum"  "%s" "\033[m", (unsigned long)window.rows - 1, (unsigned long)color, message);
    write(STDOUT_FILENO, window.screen, length);
}
...
static inline void copy_selection_to_clipboard() {
    ...
    sprintf(file->message, "copied %lu bytes", (unsigned long)buffer_length);
...
static inline void set_begin() {
    ...
    sprintf(file->message, "set begin to (%lu,%lu)", (unsigned long)file->begin.line, (unsigned long)file->begin.column);
}
