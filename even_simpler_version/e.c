...
int main(int argc, const char** argv) {
	...
	size_t capacity = 0, count = 0, cursor = 0, anchor = 0, saved = 1, mode = 1, max = 0;
...
loop:;	ssize_t r = getline(&input, &capacity, stdin);
	...
	if (mode == 1) {
		...
		if (length >= 2 and input[length - 2] == '`') { printf("%lu\n", cursor); mode = 2; }
		...
		if (not strcmp(input, "")) puts("[empty]");
		...
		else if (not strcmp(input, "print_contents")) { fwrite(text, count, 1, stdout); puts(""); } // debug
			...
			memcpy(text + anchor, text + cursor, count - cursor);
			count -= cursor - anchor;
