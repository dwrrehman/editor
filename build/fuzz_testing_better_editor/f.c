...
static inline void move_left(struct file* file) {
    ...
    if (file->render_cursor.line == file->render.count) return; // unimpl.
...
static inline void move_right(struct file* file) {
    ...
    while (file->cursor.column < line->length) {
        if ((line->line[++file->cursor.column] >> 6) != 2) break; //suspcisious code.
    }
...
///TODO: okay, now we have to do a fuzz testing on this.
/// very important.
