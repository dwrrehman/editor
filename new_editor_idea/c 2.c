#include <stdio.h>  // 2306202.165852:  a screen-based simpler editor based on the ed-like one, uses a string ds, and is modal. 
#include <stdlib.h> 
#include <string.h> 
#include <fcntl.h>
#include <unistd.h>
...
static configure_terminal(void) {
...
static void execute_shell_command(char** args) {
	pid_t pid = fork();
	if (pid < 0) { perror("fork"); exit(1); } 

	if (not pid) {
		if (execve(args[0], args, environ) == -1) { perror("execve"); exit(1); }
	} else {
		int status = 0;
...
static void do_shell(char* string) {
	tcsetattr(0, TCSAFLUSH, &terminal);
	printf("executing shell command: %s\n", string);
	execute_shell_command((char*[]){string, NULL});
...
static void sendc(void) {
	...
	else if (cliplength > 8 and not strncmp(clipboard, "execute ", 8)) { do_shell(clipboard + 8); }
...
//	printf("save: created %lub at ", count);
//	printf("%s : %s\n", directory, filename); 
//	getchar();
